...
(maxCurlSplits: Int) {
    ...
    private var mArrDropShadowVertices: Array<ShadowVertex>? = null
    ...
    private var mArrTempShadowVertices: Array<ShadowVertex>? = null
    private val mArrTempVertices: Array<Vertex>

    // Buffers for feeding rasterizer.
    private val mBufColors: FloatBuffer
    private var mBufCurlPositionLines: FloatBuffer? = null
    private var mBufShadowColors: FloatBuffer? = null
    private var mBufShadowVertices: FloatBuffer? = null
    private var mBubTextChords: FloatBuffer? = null
    private val mBufVertices: FloatBuffer

    private var mCurlPositionLinesCount: Int = 0
    private var mDropShadowCount: Int = 0
    ...
    private val mMaxCurlSplits: Int = if (maxCurlSplits < 1) 1 else maxCurlSplits
    ...
    private var mSelfShadowCount: Int = 0
    ...
    private var mVerticesCountBack: Int = 0
    private var mVerticesCountFront: Int = 0
    ...
    init {
        ...
        mBubTextChords = tbb.asFloatBuffer()
        mBubTextChords!!.position(0)
    ...
    private fun addVertex(vertex: Vertex) {
        ...
        mBubTextChords!!.put(vertex.mTexX.toFloat())
        mBubTextChords!!.put(vertex.mTexY.toFloat())
    }
...
//        }
        ...
        run {
            ...
            val dist2 = Math.sqrt((v0.mPosX - v2.mPosX) * (v0.mPosX - v2.mPosX) + (v0.mPosY - v2.mPosY) * (v0.mPosY - v2.mPosY))
            val dist3 = Math.sqrt((v0.mPosX - v3.mPosX) * (v0.mPosX - v3.mPosX) + (v0.mPosY - v3.mPosY) * (v0.mPosY - v3.mPosY))
            if (dist2 > dist3) {
                lines[1][1] = 3
                lines[2][1] = 2
            }
        ...
        for (i in 0 until mArrScanLines.size()) {
            ...
            for (j in 0 until mArrRotatedVertices.size()) {
                ...
                if (v.mPosX in scanXmin..scanXmax) {
                    ...
                    if (intersections.size() == 1 && intersections[0].mPosY > v.mPosY) {
    ...
    fun onDrawFrame(gl: GL10) {
        ...
        if (DRAW_TEXTURE && texturePage.texturesChanged) {
            ...
            var texture = texturePage.getTexture(mTextureRectFront,
                    CurlPage.SIDE_FRONT)
            ...
            if (mTextureBack) {
                ...
                texture = texturePage.getTexture(mTextureRectBack,
                        CurlPage.SIDE_BACK)
                GLUtils.texImage2D(GL10.GL_TEXTURE_2D, 0, texture, 0)
                texture.recycle()
            } else {
        ...
        gl.glTexCoordPointer(2, GL10.GL_FLOAT, 0, mBubTextChords)
...
//        }
     ...
     */
    @Synchronized
    private fun setTexCoords(left: Float, top: Float, right: Float,
                             bottom: Float) {
    ...
    private inner class Array<T>(private val mCapacity: Int) {
        private val mArray: kotlin.Array<Any?> = arrayOfNulls(mCapacity)
    ...
    private inner class ShadowVertex {
        var mPenumbraColor: Double = 0.toDouble()
        var mPenumbraX: Double = 0.toDouble()
        var mPenumbraY: Double = 0.toDouble()
        var mPosX: Double = 0.toDouble()
        var mPosY: Double = 0.toDouble()
        var mPosZ: Double = 0.toDouble()
    }
    ...
    private inner class Vertex {
        var mColor: Int = 0
        var mColorFactor: Float = 0.toFloat()
        var mPenumbraX: Double = 0.toDouble()
        var mPenumbraY: Double = 0.toDouble()
        var mPosX: Double = 0.toDouble()
        var mPosY: Double = 0.toDouble()
        var mPosZ: Double = 0.toDouble()
        var mTexX: Double = 0.toDouble()
        var mTexY: Double = 0.toDouble()
