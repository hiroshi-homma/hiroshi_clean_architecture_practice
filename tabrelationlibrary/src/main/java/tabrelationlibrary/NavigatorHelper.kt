...
class NavigatorHelper {
    ...
    private fun dispatchOnEnter(index: Int, enterPercent: Float, leftToRight: Boolean, force: Boolean) {
        if (mSkimOver || index == currentIndex || scrollState == ScrollState.SCROLL_STATE_DRAGGING
                || force) {
    ...
    private fun dispatchOnLeave(index: Int, leavePercent: Float, leftToRight: Boolean, force: Boolean) {
        if (mSkimOver ||
                index == mLastIndex || scrollState == ScrollState.SCROLL_STATE_DRAGGING ||
...
//    fun Int.onPageScrolled(positionOffset: Float) {}
